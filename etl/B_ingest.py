...
def ingest_data(base_path: str | Path) -> List[Dict]:
    ...
    if not base_path.exists() or not base_path.is_dir():
        ...
        raise FileNotFoundError(
            f"Dataset directory does not exist: {base_path}"
        )
    ...
    missing_classes = [
        ...
        if not (base_path / disease).exists()
    ]

    if missing_classes:

        logger.error(
            "Missing class folders: %s",
    ...
    if not data:
        ...
        logger.error(
            "No WAV files were found in the dataset"
        )
